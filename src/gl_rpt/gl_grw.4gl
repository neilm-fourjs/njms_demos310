...
&endif
  ...
  LET m_imagePath =
      fgl_getEnv("FGLIMAGEPATH") -- ".."||os.path.separator()||"etc"||os.path.separator()
  LET x = m_imagePath.getIndexOf(":", 2)
  IF x > 0 THEN
    LET m_imagePath = m_imagePath.subString(1, x - 1) || os.path.separator()
  END IF
  IF opts.r_orientation IS NULL THEN
    LET opts.r_orientation = "L"
  END IF
  IF opts.r_output IS NULL THEN
    LET opts.r_output = "SVG"
  END IF
...
FUNCTION glGRW_rptStart(l_filename STRING) RETURNS om.saxdocumenthandler
  ...
  LET INT_FLAG = FALSE
  IF m_setup_done IS NULL THEN
    LET m_setup_done = FALSE
  END IF
  IF NOT m_setup_done THEN
    ...
    LET opts.r_filename = l_filename
    IF NUM_ARGS() > 1 THEN
      LET opts.r_output = ARG_VAL(2)
    END IF
    IF NUM_ARGS() > 2 THEN
      LET opts.r_action = ARG_VAL(3)
    END IF
    IF NUM_ARGS() > 3 THEN
      LET r_interactive = ARG_VAL(4)
    END IF
    IF NUM_ARGS() > 4 THEN
      LET opts.r_outputFileName = ARG_VAL(5)
    END IF
    IF opts.r_action = "preview" THEN
      LET l_preview = TRUE
    END IF
    CALL glGRW_setOptions(
        l_fileName, opts.r_output, l_preview, opts.r_outputFileName, "L", r_interactive)
  END IF
  IF opts.r_output = "ASCII" THEN
    RETURN NULL
  END IF
  IF NOT r_config_valid THEN
    RETURN NULL
  END IF
    ...
    INPUT BY NAME opts.* ATTRIBUTE(WITHOUT DEFAULTS, UNBUFFERED)
      ...
      ON CHANGE r_outDataOnly
        IF NOT opts.r_outDataOnly THEN
          ...
          LET opts.r_outDataOnly = FALSE
          IF opts.r_filename != "ASCII" THEN
            CALL DIALOG.setFieldActive("r_filename", TRUE)
          END IF
  ...
  IF opts.r_filename != "ASCII" THEN
    IF NOT fgl_report_loadCurrentSettings(os.path.join(m_rptDir, opts.r_filename)) THEN
      CALL gl_winMessage(
          % "Error",
          SFMT(% "fgl_report_loadCurrentSettings(%1) Failed!", opts.r_filename),
          "exclamation")
      RETURN NULL
    END IF
  ELSE
...
FUNCTION glGRW_rptStartASCII(
    l_device STRING, l_preview STRING, l_targetName STRING, l_ori STRING)
    RETURNS om.SaxDocumentHandler
  DEFINE l_handler om.SaxDocumentHandler

  IF l_device IS NULL THEN
    LET l_device = ARG_VAL(1)
  END IF
  IF l_preview IS NULL THEN
    LET l_preview = TRUE
  END IF
  IF l_targetName IS NULL THEN
    LET l_targetName = ARG_VAL(2)
  END IF
  IF l_ori IS NULL THEN
    LET l_ori = "L"
  END IF

  IF l_device = "ASCII" THEN
    RETURN NULL
  END IF
  ...
  IF l_output = "Browser" THEN
    IF fgl_getenv("GRE_PRIVATE_DIR") IS NOT NULL THEN
      ...
      CALL fgl_report_setBrowserDocumentDirectoryURL(
          fgl_getenv("GRE_PRIVATE_URL_PREFIX") || "/" || l_uuid)
...
FUNCTION glGRW_savePDF()
  ...
  CALL ui.interface.frontcall(
      "standard", "savefile", ["", "PDF", "*.pdf", % "Save the Report"], filename)
...
FUNCTION glGRW_prnOutputRadio(l_f ui.Form)
  ...
  IF l_n IS NULL THEN
    DISPLAY "Failed to find RadioGroup!"
    RETURN
  END IF
...
FUNCTION glGRW_getPrinterList() --{{{
  ...
  WHILE NOT c.isEof()
    ...
    IF l_line.subString(1, 8) = "Printer:" THEN
      ...
      IF l_line.getCharAt(x + 1) = "T" THEN
        LET m_printers[m_printers.getLength()].default = TRUE
      END IF
      DISPLAY "p:",
          m_printers[m_printers.getLength()].name,
          " :",
          m_printers[m_printers.getLength()].default
...
FUNCTION glGRW_doubleSlash(l_line STRING) RETURNS STRING
  ...
  FOR x = 1 TO l_line.getLength()
    ...
    IF l_line.getCharAt(x) = "\\" THEN
      LET l_newline = l_newline, "\\\\"
    END IF
  END FOR
