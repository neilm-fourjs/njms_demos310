...
PRIVATE FUNCTION _check_id(id SMALLINT)
	IF id >= 1 AND id <= calendars.getLength() THEN
		IF calendars[id].field IS NOT NULL THEN RETURN END IF
	END IF
...
PRIVATE FUNCTION _week_day_name(id SMALLINT, n SMALLINT)
	...
	IF calendars[id].day_names.getLength() = 0 THEN
		FOR x = 1 TO 7
			LET	calendars[id].day_names[x] = day_shortName( MDY(1,x,2018) )
		END FOR
	END IF
...
PRIVATE FUNCTION _set_text_size(id SMALLINT)
	CASE calendars[id].cal_type
		WHEN FGLCALENDAR_TYPE_DEFAULT LET calendars[id].text_size = 10
		WHEN FGLCALENDAR_TYPE_ICON LET calendars[id].text_size = 10
		WHEN FGLCALENDAR_TYPE_TEXT LET calendars[id].text_size = 20
		WHEN FGLCALENDAR_TYPE_DOTS LET calendars[id].text_size = 20
	END CASE
END FUNCTION
--------------------------------------------------------------------------------
...
--	CALL attr.addAttribute(SVGATT_FONT_WEIGHT,	"100" )
	...
	CASE calendars[id].cal_type
		...
		WHEN FGLCALENDAR_TYPE_DEFAULT
			...
			CALL attr.addAttribute(SVGATT_FONT_WEIGHT,	"600" )
...
PRIVATE FUNCTION _month_length(y SMALLINT, m SMALLINT)
	CASE m
		WHEN	1 RETURN 31
		WHEN	2 RETURN IIF(util.Date.isLeapYear(y),29,28)
		WHEN	3 RETURN 31
		WHEN	4 RETURN 30
		WHEN	5 RETURN 31
		WHEN	6 RETURN 30
