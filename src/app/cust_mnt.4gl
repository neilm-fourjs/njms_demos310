...
MAIN
	...
	MENU
		...
		ON ACTION find LET m_func = "E"
			IF NOT query() THEN CONTINUE MENU END IF
...
FUNCTION delete() RETURNS BOOLEAN
	...
	IF NOT gl_db.gldb_checkRec(TRUE,m_rec.KEYFLD,l_stmt) THEN RETURN FALSE END IF

	IF gl_lib.gl_winQuestion(%"Confirm",%"Are you sure you want to delete this customer?","No","Yes|No","question")
		= "Yes" THEN
		LET l_stmt = "DELETE FROM "||TABNAMEQ||" WHERE "||KEYFLDQ||" = ?"
		PREPARE pre_del FROM l_stmt
		EXECUTE pre_del USING RECKEY
		MESSAGE %"Row deleted!"
		RETURN gl_db.gldb_sqlStatus(__LINE__,__FILE__,"DELETE FROM "||TABNAMEQ||" WHERE "||KEYFLDQ||" = '"||RECKEY||"'")
	END IF
	RETURN FALSE
END FUNCTION
--------------------------------------------------------------------------------
...
--		DISPLAY "Nothing changed!"
	...
	IF gl_lib.gl_winQuestion(%"Confirm",%"Update this customer?","No","Yes|No","question") = "Yes" THEN
		RETURN FALSE
	END IF

	IF m_rec.* != m_rec_o.*  THEN
		LET l_wher = KEYFLDQ||" = ?"
		LET l_stmt = gl_db.gldb_genUpdate(TABNAMEQ, l_wher, base.typeInfo.create(m_rec), base.typeInfo.create(m_rec_o),0, TRUE )
	...
	IF m_rec2.* != m_rec2_o.*  THEN
		LET l_wher = KEYFLD2Q||" = ?"
		LET l_stmt = gl_db.gldb_genUpdate(TABNAME2Q, l_wher, base.typeInfo.create(m_rec2), base.typeInfo.create(m_rec2_o),0, TRUE )
		TRY
	...
	IF m_rec3.* = m_rec3_o.* THEN
		LET l_wher = KEYFLD2Q||" = ?"
		LET l_stmt = gl_db.gldb_genUpdate(TABNAME2Q, l_wher, base.typeInfo.create(m_rec3), base.typeInfo.create(m_rec3_o),0, TRUE )
		TRY
...
FUNCTION insert(l_ad_only BOOLEAN) RETURNS BOOLEAN
	...
	IF NOT l_ad_only THEN
		LET l_stmt = "SELECT * FROM "||TABNAMEQ||" WHERE "||KEYFLDQ||" = '"||m_rec.KEYFLD||"'"
		LET m_rec_o.KEYFLD = m_rec.KEYFLD
		IF NOT gl_db.gldb_checkRec(FALSE,m_rec.KEYFLD,l_stmt) THEN RETURN FALSE END IF
		IF gl_lib.gl_winQuestion(%"Confirm",%"Insert new customer?","No","Yes|No","question") = "No" THEN
			RETURN FALSE
		END IF
	END IF
	...
	CATCH
		IF NOT gl_db.gldb_sqlStatus(__LINE__,__FILE__,l_stmt) THEN ROLLBACK WORK RETURN FALSE END IF
	END TRY
...
FUNCTION rpt1()
	...
	PREPARE r_pre FROM "SELECT * FROM "||TABNAMEQ||","||TABNAME2Q||
				" WHERE "||m_wher||" AND "||JOIN1Q||" ORDER BY "||KEYFLDQ
