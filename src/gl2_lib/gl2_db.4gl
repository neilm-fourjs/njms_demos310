...
FUNCTION (this dbInfo ) gl2_db_sqt_createdb(l_dir STRING, l_file STRING)
  ...
  IF NOT os.path.exists(l_dir) THEN
    IF NOT os.path.mkdir(l_dir) THEN
      CALL gl2_lib.gl2_exitProgram(STATUS, SFMT("DB Folder Creation Failed for: %1", l_dir))
    END IF
  END IF
...
#+
#+ @code CALL gl2_db_sqlStatus( __LINE__, "gl_db", l_sql_stmt )
#+
...
FUNCTION gl2_db_checkRec(l_ex BOOLEAN, l_key STRING, l_sql STRING) RETURNS BOOLEAN
  ...
  IF l_key IS NULL OR l_key = " " OR l_key.getLength() < 1 THEN
    CALL gl2_lib.gl2_warnPopup(% "You entered a NULL Key value!")
    RETURN FALSE
  END IF

  PREPARE gl2_db_checkrec_pre FROM l_sql
  DECLARE gl2_db_checkrec_cur CURSOR FOR gl2_db_checkrec_pre
  OPEN gl2_db_checkrec_cur
  LET l_exists = TRUE
  FETCH gl2_db_checkrec_cur
  IF STATUS = NOTFOUND THEN
    LET l_exists = FALSE
  END IF
  CLOSE gl2_db_checkrec_cur
  IF NOT l_exists THEN
    IF l_ex THEN
      CALL gl2_lib.gl2_warnPopup(% "Record '" || l_key || "' doesn't Exist!")
      RETURN FALSE
    END IF
  ELSE
    IF NOT l_ex THEN
      CALL gl2_lib.gl2_warnPopup(% "Record '" || l_key || "' already Exists!")
      RETURN FALSE
    END IF
  END IF
  RETURN TRUE
