...
FUNCTION gl2_validGDCUpdateDir() RETURNS BOOLEAN
  LET m_gdcUpdateDir = fgl_getenv("GDCUPDATEDIR")
  IF m_gdcUpdateDir.getLength() < 2 THEN
    CALL gl2_setReply(205, % "ERR", % "GDCUPDATEDIR Is not set!")
    RETURN FALSE
  END IF
  IF NOT os.Path.exists(m_gdcUpdateDir) THEN
    CALL gl2_setReply(206, % "ERR", SFMT(% "GDCUPDATEDIR '%1' Doesn't Exist", m_gdcUpdateDir))
    RETURN FALSE
  END IF
...
FUNCTION gl2_getCurrentGDC() RETURNS(STRING, STRING)
  ...
  IF NOT os.Path.exists(l_current) THEN
    CALL gl2_setReply(207, % "ERR", SFMT(% "'%1' Doesn't Exist", l_current))
    RETURN NULL, NULL
  END IF
...
-- Reads the current gdc version from current.txt file
  ...
  TRY
    ...
    CALL c.close()
  CATCH
    CALL gl2_setReply(208, % "ERR", SFMT(% "Failed to read '%1' '%2'", l_current, err_get(STATUS)))
    RETURN NULL, NULL
  END TRY
  IF l_gdcVer.getLength() < 2 THEN
    CALL gl2_setReply(209, % "ERR", SFMT(% "GDC Version is not set in '%1'!", l_gdcVer))

    RETURN NULL, NULL
  END IF
  IF l_gdcBuild.getLength() < 2 THEN
    CALL gl2_setReply(210, % "ERR", SFMT(% "GDC Build is not set in '%1'!", l_gdcBuild))
    RETURN NULL, NULL
  END IF
...
FUNCTION gl2_chkIfUpdate(l_curGDC STRING, l_newGDC STRING) RETURNS BOOLEAN
  DEFINE l_cur_maj, l_new_maj DECIMAL(4, 2)
  DEFINE l_cur_min, l_new_min SMALLINT

  CALL gl2_getVer(l_curGDC) RETURNING l_cur_maj, l_cur_min
  IF l_cur_maj = 0 THEN
    CALL gl2_setReply(
  ...
  CALL gl2_getVer(l_newGDC) RETURNING l_new_maj, l_new_min
  IF l_new_maj = 0 THEN
    CALL gl2_setReply(212, % "ERR", SFMT(% "New GDC Version is not correct format '%1'!", l_newGDC))
    RETURN FALSE
  END IF

  IF l_new_maj = l_cur_maj AND l_new_min = l_cur_min THEN
    CALL gl2_setReply(0, % "OK", % "GDC is current version")
    RETURN FALSE
  END IF

-- Is the GDC version older than the requesting GDC
  IF l_new_maj > l_cur_maj THEN
    CALL gl2_setReply(1, % "OK", SFMT(% "There is new GDC major release available: %1", l_newGDC))
    RETURN TRUE
  END IF
  IF l_new_maj = l_cur_maj AND l_new_min > l_cur_min THEN
    CALL gl2_setReply(1, % "OK", SFMT(% "There is new GDC minor release available: %1", l_newGDC))
    RETURN TRUE
  END IF
  CALL gl2_setReply(213, % "ERR", % "chkIfUpdate: Something is not right!")
  RETURN FALSE
END FUNCTION
--------------------------------------------------------------------------------
-- Sets the upd_file name and checks that it exists in the m_gdcUpdateDir
FUNCTION gl2_getUpdateFileName(l_newGDC STRING, l_gdcBuild STRING, l_gdcos STRING) RETURNS BOOLEAN
  ...
  IF NOT os.path.exists(os.path.join(m_gdcUpdateDir, l_updFile)) THEN
    CALL gl2_setReply(214, % "ERR", SFMT(% "GDC Update File '%1' is Missing!", l_updFile))
    RETURN FALSE
  END IF
...
FUNCTION gl2_setReply(l_stat INT, l_txt STRING, l_msg STRING)
