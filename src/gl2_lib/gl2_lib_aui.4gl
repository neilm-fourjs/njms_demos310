...
#+ @return ui.Window.
FUNCTION gl2_getWinNode( l_nam STRING ) RETURNS om.DomNode
...
#+ @return Node.
FUNCTION gl2_getFormNode( l_nam STRING ) RETURNS om.DomNode
	DEFINE l_frm ui.Form
	DEFINE nl om.nodeList
	DEFINE n om.domNode

	LET l_frm = gl2_getForm( NULL )
	IF l_nam IS NULL THEN
	...
	ELSE
		...
		IF nl.getLength() < 1 THEN
			CALL gl2_errMsg(__FILE__,__LINE__,"gl2_getFormNode: Form not found '"||l_nam.trim()||"'!")
			RETURN NULL
		ELSE
			LET n = nl.item(1)
		END IF
...
#+ @return ui.Form.
FUNCTION gl2_getForm( l_nam STRING ) RETURNS ui.Form
	DEFINE l_win ui.Window
	DEFINE l_frm ui.Form
...
#+ @return ui.Form.
FUNCTION gl2_genForm( l_nam STRING ) RETURNS om.DomNode
	DEFINE l_win ui.Window
	DEFINE l_frm ui.Form
...
-- create the grid, label, formfield and edit/dateedit nodes.
	...
	CALL gl2_addLabel(g, 1,2,l_msg,NULL,"big")
	GL_DBGMSG(1, "gl2_notify"||l_msg)
...
#+
#+ @return none
FUNCTION gl2_showReadMe()  RETURNS ()
	...
	CATCH
		CALL gl2_winMessage("ReadMe",SFMT(%"Open '%1' failed\n%2.",txt, err_get(STATUS)),"information")
		RETURN
	END TRY
...
#+ Example call:
#+ @code 
#+ CALL gl2_progBar(1,10,"Processing, please wait ...")   Open window and set max = 10
#+ FOR x = 1 TO 10
#+ 	CALL gl2_progBar(2,x,NULL)  Move the bar to x position
#+ END FOR
#+ CALL gl2_progBar(3,0,NULL)   Close the window
#+
...
#+ @return Nothing.
FUNCTION gl2_progBar( l_meth SMALLINT, l_curval INT, l_txt STRING ) RETURNS ()
...
-- open window and create form
	IF l_meth = 1 OR l_meth = 0 THEN
		...
		CALL gl2_addLabel(l_grid, 0,2,l_txt,NULL,NULL)
		IF l_meth = 0 THEN
...
#+ Example call:
#+ @code 
#+ CALL gl2_winInfo(1,"Processing, Please Wait ...","info")
#+ CALL doProcess()
#+ CALL gl2_winInfo(3,"","")
#+
...
#+ @return Nothing.
FUNCTION gl2_winInfo( l_meth SMALLINT, l_txt STRING, l_icon STRING ) RETURNS ()
...
-- open window and create form
	IF l_meth = 1 AND NOT m_gl_winInfo THEN
		...
		LET l_frm = gl2_genForm("gl2_wininfo")
...
#+ @param l_val value
#+ @param l_txt Text
FUNCTION gl2_addItem( l_rad om.DomNode , l_val STRING, l_txt STRING) RETURNS ()
