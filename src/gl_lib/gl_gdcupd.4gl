...
FUNCTION gl_gdcupd()
  ...
  IF NOT gl_lib_gdcupd.gl_validGDCUpdateDir() THEN
    DISPLAY gl_lib_gdcupd.m_ret.stat_txt, ":", gl_lib_gdcupd.m_ret.reply
    RETURN
  END IF
...
-- Do we have an update server defined?
  ...
  ELSE -- no update server, try a local update
    CALL gl_lib_gdcupd.gl_getCurrentGDC() RETURNING l_newGDC, l_newGDCBuild
    IF NOT gl_lib_gdcupd.gl_chkIfUpdate(l_curGDC, l_newGDC) THEN
      RETURN
    END IF
    IF NOT gl_lib_gdcupd.gl_getUpdateFileName(l_newGDC, l_newGDCBuild, l_osTarget) THEN
      RETURN
    END IF
  END IF
  ...
  DISPLAY "Stat:",
      l_stat,
      " Reply:",
      gl_lib_gdcupd.m_ret.reply,
      " ReplyStat:",
      gl_lib_gdcupd.m_ret.stat
  IF gl_lib_gdcupd.m_ret.stat != 1 THEN
    RETURN
  END IF
...
-- We have a new GDC Update ! confirm with user
  IF gl_lib.gl_winQuestion(
              "GDC Update",
              SFMT(% "%1\nUpdate Now?", gl_lib_gdcupd.m_ret.reply),
              "Yes",
              "Yes|No",
              "question")
          = "No"
...
-- does the GDC update file exist on our server
  ...
  IF NOT os.path.exists(l_localFile) THEN
    IF gl_lib_gdcupd.m_ret.upd_url IS NOT NULL THEN
      CALL gl_lib_aui.gl_winInfo(
          1,
          SFMT(% "%1\nServer Downloading Update File\nPlease Wait ...", gl_lib_gdcupd.m_ret.reply),
          "information")
      IF NOT getGDCUpdateZipFile(
              l_localFile, gl_lib_gdcupd.m_ret.upd_url, gl_lib_gdcupd.m_ret.upd_file)
          THEN
