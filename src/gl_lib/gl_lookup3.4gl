...
FUNCTION gl_lookup3( tabnam STRING, cols STRING, colts STRING, wher STRING, ordby STRING) RETURNS STRING --{{{
...
-- Open the window and define a table.
	...
	LET l_frm = gl_lib_aui.gl_genForm("gl_"||tabnam.trim() ) -- ensures form name is specific for this lookup
...
-- Loop for events.
	WHILE TRUE
		...
		CASE l_event
			WHEN "BEFORE DISPLAY"
				IF l_tot_recs = 1 THEN EXIT WHILE END IF -- if only 1 row just select it!
