...
FUNCTION gl_init(l_mdi_sdi CHAR(1), l_key STRING, l_use_fi BOOLEAN) --{{{
  ...
  END IF
  LET m_key = l_key
  IF gl_toolbar IS NULL THEN
    LET gl_toolbar = m_key
  END IF
  IF gl_topmenu IS NULL THEN
    LET gl_topmenu = m_key
  END IF

  LET l_key = fgl_getEnv("FJS_STYLE")
  IF l_key.getLength() < 2 THEN
    LET l_key = m_key
  END IF -- Style name taken from l_key
...
FUNCTION gl_setInfo(
    ...
    l_progdesc STRING,
    l_progauth STRING) --{{{
  IF l_progName IS NULL THEN
    LET l_progname = base.Application.getProgramName()
  END IF
...
FUNCTION gl_setAppInfo(l_app_name STRING, l_app_build STRING) --{{{
  ...
  IF l_app_name IS NULL THEN
    ...
    IF NOT os.path.exists(l_info_file) THEN
      LET l_info_file = "../etc/app_name.txt"
    END IF
...
--			IF gl_scr_width > C_DEF_SCR_WIDTH THEN
      IF m_windowCenter THEN
        IF l_styl = "main2" OR l_styl = "NULL" THEN
          LET l_newstyl = "centered"
        END IF
      END IF
...
--			END IF
    ...
    IF l_styl != "main" AND gl_topmenu != "default" AND l_nl.getlength() < 1
        THEN -- normal won't want default?
...
FUNCTION gl_titleWin(l_titl STRING) --{{{
  ...
  IF l_titl IS NULL OR l_titl = " " THEN
    ...
    ELSE
      IF l_titl.subString(11, 11) = ":" THEN
        LET l_titl = l_titl.subString(12, l_titl.getLength())
      END IF
    END IF
-- 01/01/1970:
-- 12345678901
...
FUNCTION gl_chkClientVer(l_cli STRING, l_ver STRING, l_feature STRING) RETURNS BOOLEAN
  ...
  IF NOT l_cli.equalsIgnoreCase(gl_fe_typ) THEN
    RETURN TRUE
  END IF
