...
-- setup stuff from environment or defaults
  ...
  IF m_dbspa IS NULL THEN
    LET m_dbdir = fgl_getenv("DBDIR")
  END IF
  IF m_dbdir IS NULL OR m_dbdir = " " THEN
    LET m_dbdir = DEF_DBDIR
  END IF

  IF m_dbspa IS NULL THEN
    LET m_dbspa = fgl_getenv("DBSPACE")
  END IF
  IF m_dbspa IS NULL OR m_dbspa = " " THEN
    LET m_dbspa = DEF_DBSPACE
  END IF

  IF m_dbdrv IS NULL THEN
    LET m_dbdrv = fgl_getenv("DBDRIVER")
  END IF
  IF m_dbdrv IS NULL OR m_dbdrv = " " THEN
    LET m_dbdrv = DEF_DBDRIVER
  END IF
      ...
      WHEN "sqt"
        IF NOT os.path.EXISTS(m_dbdir) THEN
          IF NOT os.path.mkdir(m_dbdir) THEN
            CALL gl_winMessage(
                "Error",
                SFMT("Failed to create dbdir '%1' !\n%2", m_dbdir, ERR_GET(STATUS)),
                "exclamation")
          END IF
        END IF
        LET m_dbsrc = fgl_getEnv("SQLITEDB")
        IF m_dbsrc IS NULL OR m_dbsrc = " " THEN
          LET m_dbsrc = m_dbdir || "/" || m_dbnam || ".db"
        END IF
...
FUNCTION gldb_mdb_createdb()
  ...
  LET l_sql_stmt =
      "CREATE DATABASE " || m_dbnam || " default character set utf8mb4 collate utf8mb4_unicode_ci"
  TRY
...
--TODO: Use I for smallint, integer, serial, N for numeric, decimal
  ...
  CASE l_typ.subString(1, 3)
    ...
    WHEN "CHA"
      LET l_typ = "C"
      LET l_len = gldb_getLength(l_typ)
    WHEN "VAR"
      LET l_typ = "C"
      LET l_len = gldb_getLength(l_typ)
  END CASE
