...
FUNCTION ifx_create_app_tables()
	...
	CREATE TABLE addresses(
				...
				country_code
				    CHAR(3) --		UNIQUE(line1, postal_code)
				    );
	...
	CREATE TABLE colours (
		...
		colour_hex CHAR(7)
		--,PRIMARY KEY(colour_key)
	)
	IF gl_db.m_dbtyp = "pgs" OR  gl_db.m_dbtyp = "snc" THEN
		EXECUTE IMMEDIATE "ALTER TABLE colours ADD PRIMARY KEY (colour_key)"
	END IF
	...
	IF gl_db.m_dbtyp = "pgs" OR  gl_db.m_dbtyp = "snc" THEN
		EXECUTE IMMEDIATE "ALTER TABLE quotes ADD PRIMARY KEY (quote_number)"
	END IF
