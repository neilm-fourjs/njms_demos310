...
FUNCTION ifx_create_app_tables()
	...
	CALL add_primaryKey( "customer", "customer_code" )
	...
	CALL add_primaryKey( "addresses", "rec_key" )
	...
	CALL add_primaryKey( "stock", "stock_code" )
	...
	CALL add_primaryKey( "ord_head", "order_number" )
	...
	CALL add_primaryKey( "colours", "colour_key" )
	...
	CALL add_primaryKey( "quotes", "quote_number" )
...
END FUNCTION
--------------------------------------------------------------------------------
FUNCTION add_primaryKey( l_tab STRING, l_col STRING )
	DEFINE l_stmt STRING
	DEFINE l_cmd STRING
	LET l_cmd = "PRIMARY KEY"
	IF gl_db.m_dbtyp = "ifx" THEN
		LET l_cmd = "CONSTRAINT UNIQUE"
	END IF
	LET l_stmt = SFMT("ALTER TABLE %1 ADD %2 (%3)", l_tab, l_cmd, l_col)
	DISPLAY "Excute:", l_stmt
	TRY
		EXECUTE IMMEDIATE l_stmt
	CATCH
		DISPLAY SFMT("Add Primary Key failed: %1:%2",STATUS, SQLERRMESSAGE)
	END TRY
END FUNCTION